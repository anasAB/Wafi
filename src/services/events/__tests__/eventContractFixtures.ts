...
// Single source of truth for "the one canonical fixture per event type" (WAFI-157
// design). Every contract test in this directory -- shape snapshot, subscriber
// structural/semantic checks, consumer-completeness check -- imports FIXTURES from
// here rather than building its own payload, so a producer's field rename is caught
// consistently everywhere instead of only in whichever test happened to hand-roll a
// matching fixture. `Record<DomainEventType, DomainEvent>` makes "exactly one fixture
// per event type" a compile error to violate, not a runtime check to maintain.
//
...
export const FIXTURES: Record<DomainEventType, DomainEvent> = {
  'sale.completed': {
...
// WAFI-157 design: events produced but intentionally consumed by nobody today (per
// docs/architecture/SIGNALS.md's "Known Gaps" section -- see there for the full
// rationale, not repeated here so this allowlist can't drift out of sync with its own
// stale explanation). Any DomainEventType not covered by a registered consumer AND not
// listed here fails the consumer-completeness check in eventContracts.subscribers.test.ts.
export const DORMANT_EVENTS: readonly DomainEventType[] = ['shift.opened', 'shift.closed']
