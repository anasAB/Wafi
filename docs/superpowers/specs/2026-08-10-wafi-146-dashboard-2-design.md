...
Revision: v2 — incorporates review feedback (see "Revision notes" at bottom)
...
default; tap to expand. Domain composables stay separate — this is
explicitly **not** one generic "dashboard intelligence" composable:

```
Dashboard2Screen.vue
 ├── IntelligenceCard (data from useRevenueIntelligence)
 ├── IntelligenceCard (data from useProfitIntelligence)
 ├── IntelligenceCard (data from useInventoryIntelligence)
 ├── IntelligenceCard (data from useStaffIntelligence)      -- omitted if !canViewStaffPerformance
 └── IntelligenceCard (data from useCustomerIntelligence)
```

Each internally reuses existing composables/queries where possible
(`useDashboardMetrics`, `useDeadStockReport`, `useStaffPerformanceMetrics`)
rather than re-querying from scratch.

**Data/presentation separation.** `InsightBanner.vue` (WAFI-144)
already establishes the right convention here: composables return raw
numbers, the component formats and localizes at render time via
`useI18n`'s `t()`. This spec follows the same split rather than baking
formatted strings into the data layer (baking them in would hurt
testability and block localization). Card composables return:

```ts
interface IntelligenceCardMetric {
  currentUsd: number
  previousUsd: number
...
interface IntelligenceCardDriver {
...
interface IntelligenceCardData {
  metric: IntelligenceCardMetric
  drivers: IntelligenceCardDriver[]
}
```

`IntelligenceCard.vue` (and each specific card component wrapping it)
formats the headline and each driver row via `t()`, exactly like
`InsightBanner.vue`'s `primaryLine()` does today — including showing
both the absolute values and the percentage (e.g. "842 → 691, ↓18%"),
per the review's driver-context recommendation.
...
- **Drivers:**
  - Transaction count — `invoiceCount`, already on the composable.
  - Return transaction count — **new**: `useDashboardMetrics`
  ...
  - Avg basket size — `revenue / invoiceCount`, same calc
...
- **Headline metric:** profit margin % and percentage-point change vs
  previous period.
- **Drivers (revised):**
  - Revenue Δ (reuses the Revenue card's metric — profit's own driver
    list should explain profit specifically, not repeat basket size,
    which is a revenue-composition detail, not a direct profit driver).
  - COGS Δ — **new query**: sum of `sale_line_items.quantity *
    unit_cost_usd` for the period (the exact same COGS calculation
    `useDashboardMetrics` already runs internally; just also expose it
    for the comparison period instead of discarding it).
  - Discounts Δ — **new query**: `SUM(sales.discount_usd)` current vs
    previous period (column already exists on `sales`).
- **Why this change:** discounts + basket size alone can't explain a
  profit drop driven by rising product cost. Revenue/COGS/Discounts
  covers the three levers that actually move profit and reuses data
...
- **Expand:** top offenders by frozen capital (already computed).
...
- **Discount-rate driver (revised):** instead of "flag the top
  ...
  - Example: "Top performer: Ahmed — $18,400 revenue" /
    "Highest discount rate: Sara — 6.8% (shop average 2.1%)"
...
- **Calculation/edge-case tests**, called out explicitly per the
  ...
  - `day` period, current day incomplete → drivers hidden per the
    "Day-period truncation" rule above; headline still shows per
    WAFI-144's existing behavior.
- **Component tests** per card: headline + driver rendering
  (formatted via `t()`, not baked into the composable), action-link
  routing, loading/ready/error states rendering distinctly — mirrors
