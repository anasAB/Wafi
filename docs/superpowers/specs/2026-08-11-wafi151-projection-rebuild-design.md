...
- The existing processed-event ledger (keyed by event ID, not by sequence) is the sole source of truth for "has this event been applied to this projection scope?" Incremental processing checks the ledger by event ID before applying, not a sequence threshold. Re-delivery of an already-applied event ID is a no-op, exactly as the existing durable-subscriber machinery already guarantees.
...
- **Historical backfill caveat, stated explicitly rather than assumed:** for existing rows, "the shop's timezone at that moment" is backfilled using whatever shop timezone data is available at migration time (current shop timezone, absent any stored historical timezone snapshot). This is an accepted approximation for pre-existing events, not a claim of exact historical accuracy — acceptable here because both projections in scope are commutative and not sensitive to a day boundary shifting by less than a full day in a rare historical edge case; it would not be acceptable for a future precision-sensitive projection.
...
15. After the ledger-initialization step (see Core Architectural Invariant), existing projection state and processed-event ledger entries are mutually consistent — verified by confirming incremental processing does not double-count any event that existed before this ticket's rollout.
