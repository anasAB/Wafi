...
- **Installment / layaway plans (التقسيط) with WhatsApp due-date reminders — ✅ SPECED (2026-07-14), ready for planning.** Structured payment plans (down payment + term + schedule) that post against the existing Epic 4 customer/ledger rather than a separate balance model — reuses customer, payment recording, and WhatsApp send plumbing directly. Spec: `specs/2026-07-14-installment-plans-design.md`. Pack: **Customer (+$5)** — no new SKU. **Build first** of the two (electronics table-stakes, lowest marginal cost, strongest lock-in).
- **Guided stock-take / inventory reconciliation (الجرد) with shrinkage detection — ✅ SPECED (2026-07-14), ready for planning.** Guided physical count vs system stock → variance → apply adjustments, built on the existing `stock_adjustment`/"جرد" reason from Epic 2; includes a stock-take history view with a simple shrinkage trend. Spec: `specs/2026-07-14-guided-stock-take-design.md`. Pack: **Staff (+$5)**. **Build second**, after Installment plans.
- **Product categories & subcategories (الفئات) — ✅ SPECED (2026-07-14), ready for planning.** Replaces the free-text `category` field on products with a structured, owner-managed category + optional subcategory list; adds filter/sort in the product list and a category chip filter in POS; adds a "By category" breakdown (with subcategory drill-down) to the Profit Report. Auto-migrates existing free-text category values on rollout. Spec: `specs/2026-07-14-product-categories-design.md`. Pack: **Core** (management/filtering/selling) + **Reporting** (category breakdown in Profit Report).

- **Profit Report screen — ✅ SPECED (2026-06-23) + PLANNED (2026-06-25), ready for dev.** Dedicated `/reports` screen: Week/Month/Quarter **+ custom range**, profit headline + green/red trend chart + plain breakdown, reusing the verified `useDashboardMetrics`. Spec: `specs/2026-06-23-profit-report-design.md` · Plan: `plans/2026-06-25-profit-report-screen.md` (6 TDD tasks; new `useProfitTrend` nets expenses so bars sum to headline; reuses the empty `reports` nav entry). Chosen as the next v1 build 2026-06-25 (Warehouse deferred — still v1.5, no multi-location customer yet). **Reports v2 (deferred, feedback-driven):** P&L export/PDF · best-sellers here · pre-aggregation · advanced multi-metric charts. (Superseded the quarterly-only idea below.)
