...
**Goal:** Redesign `src/pages/HomePage.vue` to match the dark SaaS mockup — responsive layout with sidebar on desktop, blue accent `#1A56DB`, ApexCharts area chart, KPI strip, health signals, and best sellers.

**Architecture:** Three independent changes land first (install dep, extend composable, new composable), then `HomePage.vue` is fully rewritten in one pass and styled section by section. All existing composables (`useDashboardMetrics`, `useBestSellers`, `useCashDrawer`, `useLowStockAlerts`, `useExchangeRate`) stay untouched except `useDashboardMetrics` which gains `invoiceCount`.

**Tech Stack:** Vue 3 + TypeScript + Tailwind (for utilities) + scoped CSS (for layout/custom dark theme) + `vue3-apexcharts` + `apexcharts`
...
|---|---|---|
| `src/features/dashboard/composables/useDashboardMetrics.ts` | Modify | Add `invoiceCount` ref and query |
| `src/features/dashboard/composables/useSalesChart.ts` | Create | Last 7 days daily sales + profit data for chart |
| `src/pages/HomePage.vue` | Full rewrite | Responsive dark dashboard with all sections |
| `src/main.ts` | No change | ApexCharts registered locally in HomePage.vue |
...
git commit -m "chore: install vue3-apexcharts and apexcharts"
```

---

## Task 2: Add `invoiceCount` to `useDashboardMetrics`

**Files:**
- Modify: `src/features/dashboard/composables/useDashboardMetrics.ts`

- [ ] **Step 1: Add `invoiceCount` ref and query**
...
```bash
git add src/features/dashboard/composables/useDashboardMetrics.ts
git commit -m "feat(dashboard): add invoiceCount to useDashboardMetrics"
```

---

## Task 3: Create `useSalesChart` composable

**Files:**
- Create: `src/features/dashboard/composables/useSalesChart.ts`
...
export interface SalesChartData {
  labels:  string[]   // Arabic day names, oldest → newest
  sales:   number[]   // daily total_usd per day
  profit:  number[]   // daily (revenue - cogs) per day
}
...
export function useSalesChart() {
  ...
  async function load() {
    ...
    // Build last 7 day date strings (oldest first)
    const days: string[] = []
    const now = new Date()
    for (let i = 6; i >= 0; i--) {
    ...
    // Daily COGS (to compute profit per day)
...
```bash
git add src/features/dashboard/composables/useSalesChart.ts
git commit -m "feat(dashboard): add useSalesChart composable for last 7 days area chart"
```

---

## Task 4: Rewrite `HomePage.vue` — Shell, Sidebar, Topbar

This task replaces the entire `HomePage.vue` with the new dark responsive shell. Sections added in later tasks will be inserted into the `.hp-body` slot.

**Files:**
- Modify: `src/pages/HomePage.vue`

- [ ] **Step 1: Replace HomePage.vue with the shell skeleton**
...
// Recent activity (last 5 cash sales from history)
...
// Nav items for sidebar + bottom nav
const navItems = [
...
<!-- NavIcon inline component — defined as a separate component below the main SFC or inline -->
<!-- See Task 4 Step 2 for NavIcon definition -->
...
- [ ] **Step 2: Create `NavIcon` helper component**

Create `src/components/ui/NavIcon.vue`:
...
<template>
  <!-- grid -->
  <svg v-if="name === 'grid'" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    <rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/>
    <rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/>
  </svg>
  <!-- cart -->
  <svg v-else-if="name === 'cart'" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    <circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/>
    <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/>
  </svg>
  <!-- doc -->
  <svg v-else-if="name === 'doc'" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
    <polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/>
  </svg>
  <!-- inventory -->
  <svg v-else-if="name === 'inventory'" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    <path d="M20 7H4a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2z"/>
    <path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/>
  </svg>
  <!-- users -->
  <svg v-else-if="name === 'users'" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/>
    <circle cx="9" cy="7" r="4"/>
    <path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>
  </svg>
  <!-- expense -->
  <svg v-else-if="name === 'expense'" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    <line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>
  </svg>
</template>
```

- [ ] **Step 3: Verify app loads at `/`**

```bash
npm run dev
```

Open http://localhost:5173. You should see a dark page with a sidebar on desktop and a mobile header. The body is mostly empty — that's expected.
...
```bash
git add src/pages/HomePage.vue src/components/ui/NavIcon.vue
git commit -m "feat(home): add responsive dark shell — sidebar, topbar, bottom nav"
```

---

## Task 5: Period Toggle + KPI Strip

**Files:**
- Modify: `src/pages/HomePage.vue` — insert sections into `.hp-body`

- [ ] **Step 1: Add period toggle + KPI strip inside `.hp-body`**

Replace the comment `<!-- SECTIONS ADDED IN LATER TASKS -->` in the template with:
...
<!-- CHART + CONTENT (later tasks) -->
```

- [ ] **Step 2: Add CSS for period row and KPI strip** — append to `<style scoped>`:
...
Open http://localhost:5173. You should see: period toggle, 4 KPI cards, sell button. Values should be 0 unless PowerSync is connected (that's fine).

- [ ] **Step 4: Commit**

```bash
git add src/pages/HomePage.vue
git commit -m "feat(home): add period toggle and KPI strip"
```
...
- [ ] **Step 1: Add the two-column content layout + chart section**

Replace `<!-- CHART + CONTENT (later tasks) -->` with:
...
<div class="hp-content-row">
  ...
  <div class="hp-col-right">
    <!-- SIGNALS (next task) -->
    <!-- EXPENSE BUTTON (next task) -->
  </div>

</div>
```

- [ ] **Step 2: Add chart computed props to `<script setup>`**

Add after `const avgPerInvoice = ...`:
...
- [ ] **Step 3: Add layout + chart CSS** — append to `<style scoped>`:
...
Open http://localhost:5173. The area chart should appear below the KPI cards. If PowerSync has no data, both lines will be flat at zero — that's correct behavior.

- [ ] **Step 5: Commit**

```bash
git add src/pages/HomePage.vue
git commit -m "feat(home): add ApexCharts area chart for last 7 days"
```

---

## Task 7: Best Sellers Table

**Files:**
- Modify: `src/pages/HomePage.vue`

- [ ] **Step 1: Replace `<!-- BEST SELLERS (next task) -->` with:**
...
- [ ] **Step 2: Add CSS** — append to `<style scoped>`:
...
Best sellers table should appear below the chart.

- [ ] **Step 4: Commit**

```bash
git add src/pages/HomePage.vue
git commit -m "feat(home): add best sellers table"
```

---

## Task 8: Health Signals Panel

**Files:**
- Modify: `src/pages/HomePage.vue`

- [ ] **Step 1: Replace `<!-- SIGNALS (next task) -->` in `.hp-col-right` with:**
...
Health signals panel should appear in the right column (desktop) or below the chart on mobile.

- [ ] **Step 4: Commit**

```bash
git add src/pages/HomePage.vue
git commit -m "feat(home): add health signals panel"
```

---

## Task 9: Live Activity Feed + Add Expense Button

**Files:**
- Modify: `src/pages/HomePage.vue`

- [ ] **Step 1: Replace `<!-- EXPENSE BUTTON (next task) -->` in `.hp-col-right` with:**
    ...
    <div v-for="sale in recentActivity" :key="sale.id" class="activity-item">
      <div class="ai-amount" dir="ltr">
        {{ sale.paymentMethod === 'cash_syp'
          ? sale.totalSyp?.toLocaleString('ar-SY') + ' ل.س'
...
Desktop view: sidebar + topbar + KPI strip + chart + best sellers in the main column, health signals + activity + expense button in the right column. Mobile: stacked single column with bottom nav. All sections visible.

- [ ] **Step 4: Commit**

```bash
git add src/pages/HomePage.vue
git commit -m "feat(home): add activity feed and add-expense button"
```
...
**Files:**
- Modify: `src/pages/HomePage.vue` (minor CSS tweaks only)
...
- [ ] **Step 2: Run dev server and verify the full golden path**
...
Checklist:
- [ ] Dark background `#06090F` everywhere — no white flash
- [ ] Sidebar visible on wide browser window (≥900px), hidden on narrow (<900px)
- [ ] Bottom nav visible on narrow, hidden on wide
- [ ] Period toggle switches metrics (اليوم / الأسبوع / الشهر)
- [ ] Rate pill opens `ExchangeRateEditor`
- [ ] KPI cards show real values when connected; zeros when offline — both acceptable
- [ ] Chart renders without console errors
- [ ] Sell button navigates to `/pos`
- [ ] Sell button disabled when no exchange rate set
- [ ] Health signals: low stock navigates to `/products?filter=low-stock`, customers to `/customers`
- [ ] Add expense button opens expense form, saves, updates metrics
- [ ] ProfitSheet opens on profit card tap
- [ ] CashDrawerSheet opens on cash card tap / cash signal tap
- [ ] Draft dialog appears if an unfinished sale exists

- [ ] **Step 3: Commit**

```bash
git add src/pages/HomePage.vue
git commit -m "feat(home): complete homepage redesign — dark SaaS dashboard with blue accent"
```
