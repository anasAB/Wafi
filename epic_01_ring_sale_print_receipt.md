...
| Reprint | إعادة طباعة |
| Duplicate copy marker | نسخة مكررة / Duplicate Copy |
| Save | حفظ |
| Clear sale | مسح |
...
- [x] **Reprinted receipts marked as copies:** `ReceiptData` (and the sale-history reprint path) carry an `isReprint: boolean` flag. When true, the printed receipt shows "نسخة مكررة / Duplicate Copy" in large bold text at the top. Non-negotiable retail anti-fraud requirement — prevents a customer reusing an old receipt to return an item twice. *Flag threading + `SimulatedDriver` marker done (commit 8bdb25e); large-bold-at-top rendering on a physical receipt is pending the real ESC/POS driver (WAFI-017 — not yet built).*
- [x] **"Clear sale" reachable at any time, not just mid-payment:** `SalePanel.vue` exposes the "مسح" (Clear sale) action (per Story 1.2's acceptance criteria) at all times the sale panel has 1+ items — including outside the payment modal. Confirmed via the existing "متأكد من حذف البيع؟" dialog, and clearing also purges the IndexedDB draft (CD-3). *(commit 8e14170)*
- [x] **Barcode scanner does not misfire on non-scanner rapid input:** The global `useBarcodeScan` keydown listener is a burst-speed detector (<33ms/char), not a focus-based guard — verify it does not fire (a) on human keyboard typing at any speed a person can sustain, and (b) does not swallow a manually-typed search query's Enter keystroke. Add an explicit test: type a search term at human speed and hit Enter — the search must submit normally, not be intercepted as a scan. *(commit d8898cf)*
